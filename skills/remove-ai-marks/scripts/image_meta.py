...
from common import atomic_write_bytes, read_bytes_input
...
def inspect_jpeg(data: bytes) -> tuple[bool, bool, list[str]]:
    ...
    while i + 4 <= n:
        ...
        # APP11 can hold JUMBF, but is not provenance by itself.
        if marker == 0xEB and _contains_any(payload, C2PA_MARKERS):
            has_c2pa = True
            findings.append("JPEG APP11 segment with C2PA marker")
...
def inspect_image(path: Path, data: bytes | None = None) -> ImageInspectReport:
    if data is None:
...
def strip_png(data: bytes, *, strip_all_text: bool = False) -> tuple[bytes, list[str]]:
    if not data.startswith(PNG_SIG):
...
def strip_jpeg(data: bytes, *, strip_all_app: bool = False) -> tuple[bytes, list[str]]:
    if not data.startswith(JPEG_SOI):
    ...
    while i < n:
        ...
        if 0xE0 <= marker <= 0xEF:  # APPn
            if marker == 0xEB and _contains_any(payload, C2PA_MARKERS):
                drop = True
                actions.append("drop APP11 with C2PA marker")
            ...
            if strip_all_app or _contains_any(payload, AI_META_HINTS + C2PA_MARKERS):
                drop = True
                actions.append("drop matching COM comment")
            else:
                keep = True
        else:
            keep = True
...
def clean_image(
    ...
    *,
    ...
    after = inspect_image(dest, cleaned)
    return {
        "input": str(path),
        "output": str(dest),
        "format": fmt,
